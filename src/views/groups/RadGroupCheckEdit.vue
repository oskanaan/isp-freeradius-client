<template>
  <v-container>
    <h1>{{this.$t(`radGroupCheckEditTitle`)}} {{ groupName }}</h1>
...
export default {
  ...
  methods: {
    fetchAttributes() {
      this.loading = true;
      apiService.getGroupCheckByGroupName(this.groupName, this.page, this.pageSize)
          .then(response => {
            this.replies = response.data._embedded.radgroupcheck;
    ...
    addAttribute() {
      apiService.addGroupCheck(this.groupName, this.attributeName, this.operator, this.value)
          .then(() => {
    ...
    updateAttribute() {
      apiService.updateGroupCheck(this.id, this.groupName, this.attributeName, this.operator, this.value)
          .then(() => {
            this.fetchAttributes();
            this.attributeName = '';
            this.operator = '';
    ...
    deleteSelectedAttributes() {
      if (this.selectedRadChecks.length > 0) {
        apiService.deleteGroupChecks(this.selectedRadChecks)
            .then(() => {
              this.fetchAttributes();
