<template>
  <v-container>
    <h1 class="mb-5">{{ this.$t(`adjustInvoice`) }} </h1>

    <v-card class="elevation-12 mb-5">
      <v-card-title class="bg-primary white--text">{{ this.$t('adjustmentValue') }}</v-card-title>

      <v-row class="mx-3 no-gutters">
        <v-col cols="4">
          <h3>{{ this.$t(`currentTotalCost`, {cost: this.totalCost}) }}</h3>
          <h3>{{ this.$t(`totalCostAfterAdjustment`, {cost: (parseFloat(this.totalCost) + parseFloat(this.adjustmentAmount))}) }}</h3>
        </v-col>
      </v-row>
      <v-row class="mx-3 no-gutters">
        <v-col cols="4">
          <v-text-field v-model="adjustmentAmount" :label="this.$t('adjustmentAmount')" required dense :error-messages="fieldErrors['adjustmentAmount']"/>
        </v-col>
...
export default {
  ...
  methods: {
    adjust() {
      ...
      apiService.adjustInvoice(this.invoiceId, this.adjustmentAmount)
          .then(() => {
