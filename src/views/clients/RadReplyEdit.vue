<template>
  <v-container>
    <h1 class="mb-5">{{ this.$t(`radReplyEditTitle`) }} {{ username }}</h1>
    ...
    <v-data-table-server
      ...
      <template v-slot:item="{ item }">
        <tr>
          <td><v-checkbox v-model="selectedRadChecks" :value="item" hide-details="true"></v-checkbox></td>
  ...
  methods: {
    fetchAttributes() {
      if (this.username !== undefined && this.username !== '') {
        this.loading = true;
        apiService.getRadReplyAttributes(this.username, this.page, this.pageSize)
            .then(response => {
              console.log(response)
              this.attributes = response.data._embedded.radreply;
    ...
    addAttribute() {
      apiService.addRadReplyAttribute(this.username, this.attributeName, this.operator, this.value)
          .then(() => {
    ...
    updateAttribute() {
      apiService.updateRadReplyAttribute(this.id, this.username, this.attributeName, this.operator, this.value)
          .then(() => {
            this.fetchAttributes();
            this.attributeName = '';
            this.operator = '';
    ...
    deleteSelectedAttributes() {
      if (this.selectedRadChecks.length > 0) {
        apiService.deleteRadReplyAttributes(this.selectedRadChecks)
            .then(() => {
              this.fetchAttributes();
